...
public class GalleryServiceImplV2 extends GalleryServiceImplV1{
	 ...
	 * 
	 * 이떄 각 매개변수의 요소 type 앞에 작성해 주면된다.
	 *  
	 */
	 ...
	 * 첨부파일이 있는 게시물 삭제하기
	 * 1. 첨부파일을 제거 후 
	 *      가. 첨부파일을 삭제하기 위하여 데이터를 다시 select하여 첨부파일 이름을 가져오기
     *      나. 가져온 이름으로 파일을 삭제하고
	 * 2. 게시물 데이터를 삭제
	 * 
	 * 첨부파일 뿐만 아니라 JOIN 된 데이터가 또 있다!
	...
	public int delete(Long g_seq) {
		...
		for(FileDTO file : fileList) {
			...
			// 첨부파일 삭제
			String attFileName = file.getFile_upname();// 실제로 저장되어있는 파일명
	    ...
	    if(ret > 0){
	    	...
	    	log.debug("파일 삭제 실패로 데이터 삭제하지 않음");
	    }
