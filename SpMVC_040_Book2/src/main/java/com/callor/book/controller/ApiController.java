 ...
 * 
 * 이 경우 절대 view를 return 하지 않는다.
 * return 되는 type로 데이터를 response 한다.
...
public class ApiController {
	...
	public String reqAjax(@RequestBody Map<String, String> maps) { //Map보다 VO를 권장하긴 한다!! VO로 하는법 나중에 알려준대
		log.debug("Maps {}", maps.toString());
		log.debug("Search {}", maps.get("search"));
		log.debug("Search {}", maps.get("st_name"));
		return "OK";
	}
}
